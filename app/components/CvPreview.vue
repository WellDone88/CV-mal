<template>
  <div class="w-full max-w-[800px] bg-white min-h-[1050px] shadow-xl rounded-sm p-12 space-y-8 print:shadow-none print:p-0 print:w-full">
    ...
    <div class="border-b-4 border-indigo-600 pb-6">
    ...
    <div v-if="cvData.profil" class="space-y-2">
      <h2 class="text-lg font-bold text-slate-900 uppercase tracking-wider text-sm border-b pb-1">Profil</h2>
      <p class="text-sm text-slate-600 leading-relaxed whitespace-pre-line">{{ cvData.profil }}</p>
    </div>

    <div class="space-y-4">
      <h2 class="text-lg font-bold text-slate-900 uppercase tracking-wider text-sm border-b pb-1">Arbeidserfaring</h2>
